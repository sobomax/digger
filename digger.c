...
static struct digger {
  int16_t h, v, rx, ry, mdir, bagtime, rechargetime, deathstage, deathbag,
      deathani, deathtime, emocttime, emn, msc, lives, ivt;
...
void initdigger(void) {
  ...
  for (dig = dgstate.curplayer; dig < dgstate.diggers + dgstate.curplayer;
       dig++) {
...
void dodigger(struct digger_draw_api *ddap) {
  int n;
  int16_t tdir;
...
static void updatefire(struct digger_draw_api *ddap, int n) {
  int16_t pix = 0, fx, fy;
  ...
  if (digdat[n].notfiring) {
    if (digdat[n].rechargetime != 0) {
      ...
      if (getfirepflag(n - dgstate.curplayer)) {
        if (digdat[n].dob.alive) {
          ...
          bullet_obj_init(&digdat[n].bob, n - dgstate.curplayer,
                          digdat[n].dob.dir, fx, fy);
    ...
    i = clfirst[4];
    while (i != -1) {
      if (i - FIRSTDIGGER + dgstate.curplayer != n &&
          !digdat[i - FIRSTDIGGER + dgstate.curplayer].invin &&
    ...
    if (clfirst[0] != -1 || clfirst[1] != -1 || clfirst[2] != -1 ||
        clfirst[3] != -1 || clfirst[4] != -1)
      clflag = true;
    else
      clflag = false;
    if (clfirst[0] != -1 || clfirst[1] != -1 || clfirst[3] != -1) {
      ...
      while (i != -1) {
        if (digdat[i - FIRSTFIREBALL + dgstate.curplayer].bob.expsn == 0) {
          CALL_METHOD(&digdat[i - FIRSTFIREBALL + dgstate.curplayer].bob,
                      explode);
        }
        i = clcoll[i];
      }
...
static void updatedigger(struct digger_draw_api *ddap, int n) {
  ...
  if (bagf) {
    if (digdat[n].mdir == DIR_RIGHT || digdat[n].mdir == DIR_LEFT) {
      ...
      digdat[n].bagtime++;
    } else if (!pushudbags(ddap, clfirst, clcoll))
      push = false;
...
static void diggerdie(struct digger_draw_api *ddap, int n) {
  int clfirst[TYPES], clcoll[SPRITES], i;
  bool alldead;
  ...
  case 1:
    ...
    drawdigger(n - dgstate.curplayer, 15, digdat[n].dob.x, digdat[n].dob.y,
               false);
    incpenalty();
  ...
  case 2:
    ...
    drawdigger(n - dgstate.curplayer, 14 - digdat[n].deathani, digdat[n].dob.x,
               digdat[n].dob.y, false);
    for (i = 0; i < TYPES; i++)
      clfirst[i] = first[i];
    for (i = 0; i < SPRITES; i++)
      clcoll[i] = coll[i];
  ...
  case 4:
    if (digdat[n].deathtime != 0)
      ...
      if (dgstate.diggers == 1 && !sounddiedone && musicflag) {
        frame -= 1;
        break;
      }
      digdat[n].dead = true;
...
static void initbonusmode(struct digger_draw_api *ddap) {
  int i;
  bonusmode = true;
  erasebonus(ddap);
  ddap->ginten(1);
    ...
    if (digdat[n].dob.alive)
      if (digdat[n].mdir == DIR_UP || digdat[n].mdir == DIR_DOWN)
        if ((digdat[n].dob.x - 12) / 20 == h)
          if ((digdat[n].dob.y - 18) / 18 == v ||
              (digdat[n].dob.y - 18) / 18 + 1 == v)
            return true;
  return false;
}
...
static bool getfirepflag(int n) { return n == 0 ? firepflag : fire2pflag; }

int diggerx(int n) { return digdat[n].dob.x; }

int diggery(int n) { return digdat[n].dob.y; }

bool digalive(int n) { return digdat[n].dob.alive; }

void digresettime(int n) { digdat[n].bagtime = 0; }
...
int getlives(int pl) { return digdat[pl].lives; }
