...
bool escape = false, firepflag = false, fire2pflag = false, pausef = false,
     mode_change = false;
bool krdf[NKEYS] = {false, false, false, false, false, false,
                    false, false, false, false, false, false,
                    false, false, false, false, false, false};

static bool aleftpressed = false, arightpressed = false, auppressed = false,
            adownpressed = false, start = false, af1pressed = false;
static bool aleft2pressed = false, aright2pressed = false, aup2pressed = false,
            adown2pressed = false, af12pressed = false;

int16_t akeypressed;

static int16_t dynamicdir = -1, dynamicdir2 = -1, staticdir = -1,
               staticdir2 = -1, joyx = 0, joyy = 0;

static bool joybut1 = false;

static int16_t keydir = 0, keydir2 = 0, jleftthresh = 0, jupthresh = 0,
               jrightthresh = 0, jdownthresh = 0, joyanax = 0, joyanay = 0;
...
void checkkeyb(void) {
  int i, j, k = 0;
  bool *aflagp[10] = {&arightpressed, &auppressed,    &aleftpressed,
                      &adownpressed,  &af1pressed,    &aright2pressed,
                      &aup2pressed,   &aleft2pressed, &adown2pressed,
                      &af12pressed};
  input_poll_async();
  if (leftpressed)
    aleftpressed = true;
...
void flushkeybuf(void) {
  ...
  aleft2pressed = aright2pressed = aup2pressed = adown2pressed = af12pressed =
      false;
}
...
bool ou2pressed = false, od2pressed = false, ol2pressed = false,
     or2pressed = false;
...
void readdirect(int n) {
  ...
  if (n == 0) {
    ...
    if ((ou2pressed && !u2 && dynamicdir2 == DIR_UP) ||
      ...
      if (u2)
        dynamicdir2 = staticdir2 = 2;
      if (d2)
        dynamicdir2 = staticdir2 = 6;
      if (l2)
        dynamicdir2 = staticdir2 = 4;
      if (r2)
        dynamicdir2 = staticdir2 = 0;
    }
    ou2pressed = u2;
    od2pressed = d2;
    ol2pressed = l2;
...
/* Why the joystick reading is split between readdirect and getdir like this is
   a mystery to me. */
