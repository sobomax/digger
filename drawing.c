...
static uint8_t monbufs[MONSTERS][480], bagbufs[BAGS][480],
    bonusbufs[BONUSES][480], diggerbufs[DIGGERS][480], firebufs[FIREBALLS][128];

static uint16_t bitmasks[12] = {0xfffe, 0xfffd, 0xfffb, 0xfff7, 0xffef, 0xffdf,
...
static void outtextl(struct digger_draw_api *ddap, const char *p, int16_t x,
                     int16_t y, int16_t c, int16_t l) {
  int16_t i;
...
void outtext(struct digger_draw_api *ddap, const char *p, int16_t x, int16_t y,
             int16_t c) {

  outtextl(ddap, p, x, y, c, strlen(p));
}

void erasetext(struct digger_draw_api *ddap, int16_t n, int16_t x, int16_t y,
               int16_t c) {
...
void eatfield(int16_t x, int16_t y, int16_t dir) {
  int16_t h = (x - 12) / 20, xr = ((x - 12) % 20) / 4, v = (y - 18) / 18,
          yr = ((y - 18) % 18) / 3;
  incpenalty();
  switch (dir) {
  case DIR_RIGHT:
...
void drawemerald(int16_t x, int16_t y) {
  ...
  if (sdl_get_lighting()) {
    extern void sdl_add_light(int x, int y, int r, int g, int b, int radius);
    sdl_add_light(x + 4, y + 5, 100, 255, 150, 24);
  }
}
...
void drawfire(int n, int16_t x, int16_t y, int16_t t) {
  ...
  if (sdl_get_lighting()) {
    extern void sdl_add_light(int x, int y, int r, int g, int b, int radius);
    if (t == 0) {
      sdl_add_light(x + 4, y + 4, 255, 120, 40, 16);
    } else {
      int r = 255;
      int g = 255 - t * 42;
      int b = 220 - t * 55;
      if (g < 60) g = 60;
      if (b < 40) b = 40;
      sdl_add_light(x + 4, y + 4, r, g, b, 20 + t * 4);
    }
  }
}
...
void drawdigger(int n, int16_t t, int16_t x, int16_t y, bool f) {
  ...
  if (digspr[n] < 0)
    digspr[n] = 0;
  if (t >= 0 && t <= 6 && !(t & 1)) {
    initspr(FIRSTDIGGER + n, (t + (f ? 0 : 1)) * 3 + digspr[n] + 1 + nn, 4, 15,
            0, 0);
